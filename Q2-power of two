...
powerof2 <- function(lambda, seed){
  ...
  for(j in 2:50000){
    ...
    Q <- c()
    option <- sample(10, 2, replace = FALSE)
    Q[1] <- findQX(j,t[j], depart_list[option[1]])
    Q[2] <- findQX(j,t[j], depart_list[option[2]])
    choose <- option[which.min(Q)]
